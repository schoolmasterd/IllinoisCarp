...
PSI_bhcp<-read.csv("psi_bhcp.csv")
...
make_x<-function(who){
  ...
  hold_matrix[a] <- 1 - colSums(hold_matrix, na.rm = T)
  #hold_matrix<-t(hold_matrix)
  return(hold_matrix)
}
...
pc_Dresden<-matrix(NA,nrow=1000,ncol=6)
ans_Dres<-list()
